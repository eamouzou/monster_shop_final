  ...
  context 'I can edit a discount' do
    before :each do
      ...
      within "#discount-#{@fiveoff.id}" do
        click_link("Edit #{@fiveoff.name}")
        expect(current_path).to eq("/merchant/#{@merchant_user.merchant.id}/discounts/#{@fiveoff.id}/edit")
      end
    end
    ...
    it "edits discount" do
      ...
      expect(page).to have_content("The current percentage is 5.")
      expect(page).to have_content("The threshold is the quantity of a single item required for the discount to be activated.")
      expect(page).to have_content("The current threshold is 10.")
      ...
      expect(current_path).to eq("/merchant/#{@merchant_user.merchant.id}/discounts/#{last_discount.id}")
      expect(page).to have_content("Name: 60% off 50 Items or More")
      expect(page).to have_content("Percentage Off: 60")
      expect(page).to have_content("Quantity of Single Item Required for Discount? 50")
      expect(page).to have_link("Edit 60% off 50 Items or More")
      expect(page).to have_link("Delete 60% off 50 Items or More")
      expect(page).to have_link("All Discounts")
      click_link("All Discounts")
      expect(current_path).to eq("/merchant/#{@merchant_user.merchant.id}/discounts")
    end

    it "flashes error message if edit form filled out incorrectly" do
      name = nil
      percentage = -2
      threshold = 0
