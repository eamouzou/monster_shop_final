...
RSpec.describe 'As an merchant employee', type: :feature do
  context 'I can access the discounts index' do
    before :each do
    ...
    it "see links to add, show, edit, and delete discount" do
      expect(page).to have_link('Add New Discount')

      within "#discount-#{@twentyoff.id}" do
        expect(page).to have_link("Edit #{@twentyoff.name}")
        expect(page).to have_link("Delete #{@twentyoff.name}")
        expect(page).to have_link("Show #{@twentyoff.name}")
        expect(page).to have_content("Percentage Off: 20.00%")
        expect(page).to have_content("Quantity of Single Item Required for Discount? 30")
      end

      within "#discount-#{@fiveoff.id}" do
        expect(page).to have_link("Edit #{@fiveoff.name}")
        expect(page).to have_link("Delete #{@fiveoff.name}")
        expect(page).to have_link("Show #{@fiveoff.name}")
        expect(page).to have_content("Percentage Off: 5.00%")
        expect(page).to have_content("Quantity of Single Item Required for Discount? 10")
      end

    end

    it "add link take user to add page" do
      click_link('Add New Discount')
      expect(current_path).to eq("/merchant/#{@merchant_user.merchant.id}/discounts/new")
    end

    it "edit link take user to edit page" do
      within "#discount-#{@twentyoff.id}" do
        click_link("Edit #{@twentyoff.name}")
        expect(current_path).to eq("/merchant/#{@merchant_user.merchant.id}/discounts/#{@twentyoff.id}/edit")
      end
    end

    it "show link takes user to show page" do
      within "#discount-#{@twentyoff.id}" do
        click_link("Show #{@twentyoff.name}")
        expect(current_path).to eq("/merchant/#{@merchant_user.merchant.id}/discounts/#{@twentyoff.id}")
      end
    end

    it "delete link take user to edit page" do
      within "#discount-#{@twentyoff.id}" do
        expect(page).to have_link("Delete #{@twentyoff.name}")
      end
    end
