  ...
  context 'I can edit a discount' do
    before :each do
      ...
      within "#discount-#{@fiveoff.id}" do
        click_link("Delete #{@fiveoff.name}")
      end
    end

    it "has removed deleted discount information" do
      expect(current_path).to eq("/merchant/#{@merchant_user.merchant.id}/discounts")

      expect(page).to_not have_content("Name: 5%off10ItemsOrMore")
      expect(page).to_not have_content("Percentage Off: 5")
      expect(page).to_not have_content("Quantity of Single Item Required for Discount? 10")
      expect(page).to_not have_link("Edit 5%off10ItemsOrMore")
      expect(page).to_not have_link("Delete 5%off10ItemsOrMore")
    end
  end
end
